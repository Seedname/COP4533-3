...
def hvlcs(character_map, A, B) -> list[list[int]]:
    ...
    for i in range(1, len(A) + 1):
        for j in range(1, len(B) + 1):
            # if the two characters match, add the value of the character to the dp value of the previous characters
            if A[i - 1] == B[j - 1]:
                dp[i][j] = dp[i - 1][j - 1] + character_map[A[i - 1]]
            # take the maximum of the previous characters and the current value
